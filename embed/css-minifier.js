...
(function () {
  'use strict';
  var TARGET_ID = 'ta-css-minifier';
  var BASE = 'https://toolaspect.com/css-minifier/';
  var URLS = [
    'https://cdn.jsdelivr.net/npm/csso@5.0.5/dist/csso.js',
    'https://unpkg.com/csso@5.0.5/dist/csso.js'
  ];
  var PRESETS = {
    keep: { label: 'Keep structure', opts: { restructure: false } },
  ...
  styleEl.setAttribute('data-ta-embed', 'css-minifier');
  styleEl.textContent = CSS;
  if (!document.querySelector('style[data-ta-embed="css-minifier"]')) {
  ...
  root.innerHTML = ''
    + '<div class="ta-embed-title">CSS Minifier</div>'
    + '<div class="ta-embed-subtitle">csso 5.0.5 in the visitor&rsquo;s browser &mdash; nothing uploads</div>'
    + '<div class="ta-embed-card">'
    + '<label>Preset</label><select class="ta-preset">'
    + '<option value="keep">Keep structure (whitespace only)</option>'
    + '<option value="restructure" selected>Restructure (merge rules)</option>'
    + '</select>'
    + '<label>CSS in</label><textarea class="ta-src" spellcheck="false" placeholder="Paste CSS here&hellip;"></textarea>'
  ...
  function loadEngine() {
    ...
    loaded = tryNext().then(function () {
      if (!window.csso) throw new Error('csso global missing');
      return window.csso;
    });
    return loaded;
  }
  ...
  function go() {
    ...
    if (!code.trim()) {
      ...
      return note('Paste CSS to minify.');
    }
    note('Minifying…');
    loadEngine().then(function (csso) {
      var r = csso.minify(code, PRESETS[root.querySelector('.ta-preset').value].opts);
      var m = bytes(r.css);
      root.querySelector('.ta-out').value = r.css;
      root.querySelector('.ta-res').textContent = nfmt(m);
      root.querySelector('.ta-saved').textContent = nfmt(n - m) + ' (' + ((n - m) / n * 100).toFixed(1) + '%)';
      note('Done — csso (MIT) ran locally.', 'ok');
    }).catch(function (e) {
      root.querySelector('.ta-out').value = '';
      root.querySelector('.ta-res').textContent = '—';
      root.querySelector('.ta-saved').textContent = '—';
  ...
  root.querySelector('.ta-dl').addEventListener('click', function () {
    ...
    a.href = URL.createObjectURL(new Blob([out], { type: 'text/css' }));
    a.download = 'minified.css';
    document.body.appendChild(a); a.click(); document.body.removeChild(a);
    setTimeout(function () { URL.revokeObjectURL(a.href); }, 4000);
    note('Downloaded minified.css.', 'ok');
  });

  window.ToolAspectEmbeds = window.ToolAspectEmbeds || {};
  window.ToolAspectEmbeds.cssMinifier = { run: go };
})();
