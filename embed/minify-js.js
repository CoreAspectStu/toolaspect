...
(function () {
  'use strict';
  var TARGET_ID = 'ta-minify-js';
  var BASE = 'https://toolaspect.com/minify-js/';
  var URLS = [
    'https://cdn.jsdelivr.net/npm/terser@5.51.2/dist/bundle.min.js',
    'https://unpkg.com/terser@5.51.2/dist/bundle.min.js'
  ];
  var PRESETS = {
    strip: { label: 'Strip only', opts: { compress: false, mangle: false, format: { comments: false } } },
    compress: { label: 'Compress', opts: { mangle: false, format: { comments: false } } },
    full: { label: 'Compress + mangle', opts: { toplevel: true } }
  };
  ...
  styleEl.setAttribute('data-ta-embed', 'minify-js');
  styleEl.textContent = CSS;
  if (!document.querySelector('style[data-ta-embed="minify-js"]')) {
  ...
  root.innerHTML = ''
    + '<div class="ta-embed-title">Minify JS</div>'
    + '<div class="ta-embed-subtitle">terser 5.51.2 in the visitor&rsquo;s browser &mdash; nothing uploads</div>'
    + '<div class="ta-embed-card">'
    + '<label>Preset</label><select class="ta-preset">'
    + '<option value="strip">Strip only (comments + whitespace)</option>'
    + '<option value="compress">Compress (no renaming)</option>'
    + '<option value="full" selected>Compress + mangle (smallest)</option>'
    + '</select>'
    + '<label>JavaScript in</label><textarea class="ta-src" spellcheck="false" placeholder="Paste JavaScript here&hellip;"></textarea>'
  ...
  function loadEngine() {
    ...
    loaded = tryNext().then(function () {
      if (!window.Terser) throw new Error('terser global missing');
  ...
  function go() {
    ...
    if (!code.trim()) {
      ...
      return note('Paste JavaScript to minify.');
    }
    note('Minifying…');
    loadEngine().then(function (T) {
      return T.minify(code, PRESETS[root.querySelector('.ta-preset').value].opts).then(function (r) {
        if (r.error) throw r.error;
        var m = bytes(r.code);
        root.querySelector('.ta-out').value = r.code;
        root.querySelector('.ta-res').textContent = nfmt(m);
        root.querySelector('.ta-saved').textContent = nfmt(n - m) + ' (' + ((n - m) / n * 100).toFixed(1) + '%)';
        note('Done — terser (BSD-2-Clause) ran locally.', 'ok');
      });
    }).catch(function (e) {
      root.querySelector('.ta-out').value = '';
      root.querySelector('.ta-res').textContent = '—';
  ...
  root.querySelector('.ta-dl').addEventListener('click', function () {
    ...
    note('Downloaded minified.js.', 'ok');
  });

  window.ToolAspectEmbeds = window.ToolAspectEmbeds || {};
  window.ToolAspectEmbeds.minifyJs = { run: go };
})();
